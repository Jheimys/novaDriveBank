...
def load_scalers(df, nome_colunas):
    for nome_coluna in nome_colunas:
        nome_arquivos_scaler = f"./objects/scaler{nome_coluna}.joblib"
        scaler = joblib.load(nome_arquivos_scaler)
        df[nome_coluna] = scaler.transform(df[[nome_coluna]])
    return df

def load_encoders(df, nome_colunas):
    for nome_coluna in nome_colunas:
        nome_arquivos_encoder = f"./objects/labelencoder{nome_coluna}.joblib"
        label_encoder = joblib.load(nome_arquivos_encoder)
